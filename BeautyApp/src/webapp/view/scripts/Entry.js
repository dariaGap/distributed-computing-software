...
async function getPossibleTime() {
    ...
    let addr = "http://localhost:8080/free-time?"
    if (selectedMaster >= 0) {
...
async function getMasters() {
    ...
    let addr = "http://localhost:8080/masters"
...
async function getServices() {
    ...
    let selected = [];
    let addr = "http://localhost:8080/services?"
    if (selectedMaster >= 0) {
        addr += "master=" + document.getElementById("choiceMaster").value;
        addr += "&"
    }
...
async function go() {
    ...
    let addr = "http://localhost:8080/save?date=" + date
