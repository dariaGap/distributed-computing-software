...
async function openComplaint() {
    ...
    let transferButton = document.getElementById("saveComplaintForm");
    transferButton.hidden = false;
    if (complaintInfo["state"] != "WAIT_EXPLANATORY") {
    ...
    complaintExplanatory.readOnly = true;
    ...
    if (complaintInfo["state"] == "WAIT_EXPLANATORY") {
        complaintExplanatory.readOnly = false;
    }
}

async function saveExplanatory() {
    let addr = "/master/addExplanatory?id="
        + document.getElementById("complaints").value
        + "&expl=" + document.getElementById("complaintExplanatory").value;
    await fetch(addr)
        .then(response => {
            if (response.status == 200) {
                alert("Объяснительная сохранена");
                let complaintBlockId = document.getElementById("complaintInfoBlock");
                complaintBlockId.hidden = true;
                getComplaints();
            } else {
...
var f = document.getElementById('saveComplaintForm')
